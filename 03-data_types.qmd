...
- [Datakehikko (data.frame)](#sec-data-frame)
- [Matriisi (matrix)](#sec-matrix)
- [Taulukko (array)](#sec-array)
- [Lista (list)](#sec-list)

Datakehikko on R:n objekti, jossa voidaan säilyttää aineistoa. Muuttujat ovat kehikon sarakkeita ja havaintoyksiköt rivejä. Datakehikossa jokaisella muuttujilla tulee aina olla nimi. Datakehikko on tyypillin tietotyyppi erilaisten aineistojen käsittelyyn, jotka sisältävät useamman kuin yhden muuttujan.
...
no_names_data
```
Datakehikkojen käsittelystä kerrotaan tarkemmin luvussa [Datan muokkaaminen](#sec-data-frame-wrangling).
...
iris
```
![](files/iris_print_data.png)

```{r, eval=FALSE}
# Print 6 first rows of iris data
head(iris)
```
![](files/iris_print_head_data.png)
...
head(iris, 2)
```
![](files/iris_print_head2_data.png)
